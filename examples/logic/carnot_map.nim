...
proc setCarnotMapGlobals*(globals: EntityId) =
  doc.update globals:
    ...
    add CanvasSettings(autoSize: true, margin: v2(1))
...
proc drawCarnotMap*(doc: World, variables: seq[string], data: seq[seq[int]], cellSize: float = 2.0) =
  let tableRows = if variables.len >= 4: 4 else: 2
  let tableSize = v2(4.0 * cellSize, float(tableRows) * cellSize)
  ...
  doc.forEach (r: RectTable, pos: Position2||point2()):
    ...
    let bH = cs * 0.125
    let bW = bH
    let sz = v2(r.size.x, r.size.y)

    proc drawLabel(name: string, labelPos: Point2, anchor: PositionAt) =
      var name = name
      if name.startsWith("!"):
        let v = case anchor
          of PositionAtBottom: v2(0, -(1 - 0.1))
          of PositionAtTop:    v2(0, -0.1)
    ...
    if variables.len == 3:
      doc.add FigureBracket(a: pos + v2(0, 0) * sz, b: pos + v2(0.5, 0) * sz, h: v2(0, -bH), power: 2)
      doc.add FigureBracket(a: pos + v2(0.5, 0) * sz, b: pos + v2(1, 0) * sz, h: v2(0, -bH), power: 2)
      doc.add FigureBracket(a: pos + v2(0.25, 1) * sz, b: pos + v2(0.75, 1) * sz, h: v2(0, bH), power: 2)
      drawLabel "!" & variables[0], pos + v2(0, 1/4) * sz + v2(-0.2, 0),       PositionAtRight
      drawLabel variables[0],       pos + v2(0, 3/4) * sz + v2(-0.2, 0),       PositionAtRight
      drawLabel "!" & variables[1], pos + v2(0.25, 0) * sz + v2(0, -(bH+0.1)), PositionAtBottom
      drawLabel variables[1],       pos + v2(0.75, 0) * sz + v2(0, -(bH+0.1)), PositionAtBottom
      drawLabel "!" & variables[2], pos + v2(1/8, 1) * sz + v2(0, bH+0.1),     PositionAtTop
      drawLabel "!" & variables[2], pos + v2(7/8, 1) * sz + v2(0, bH+0.1),     PositionAtTop
      drawLabel variables[2],       pos + v2(0.5, 1) * sz + v2(0, bH+0.1),     PositionAtTop

    elif variables.len >= 4:
      doc.add FigureBracket(a: pos + v2(0, 0) * sz,    b: pos + v2(0.5, 0) * sz,    h: v2(0, -bH),  power: 2)
      doc.add FigureBracket(a: pos + v2(0.5, 0) * sz,  b: pos + v2(1, 0) * sz,      h: v2(0, -bH),  power: 2)
      doc.add FigureBracket(a: pos + v2(0.25, 1) * sz, b: pos + v2(0.75, 1) * sz,   h: v2(0, bH),   power: 2)
      doc.add FigureBracket(a: pos + v2(0, 0) * sz,    b: pos + v2(0, 0.5) * sz,    h: v2(-bW, 0),  power: 2)
      doc.add FigureBracket(a: pos + v2(0, 0.5) * sz,  b: pos + v2(0, 1) * sz,      h: v2(-bW, 0),  power: 2)
      doc.add FigureBracket(a: pos + v2(1, 0.25) * sz, b: pos + v2(1, 0.75) * sz,   h: v2(bW, 0),   power: 2)
      drawLabel "!" & variables[2], pos + v2(0.25, 0) * sz + v2(0, -(bH+0.1)),  PositionAtBottom
      drawLabel variables[2],       pos + v2(0.75, 0) * sz + v2(0, -(bH+0.1)),  PositionAtBottom
      drawLabel "!" & variables[3], pos + v2(1/8, 1) * sz + v2(0, bH+0.1),      PositionAtTop
      drawLabel "!" & variables[3], pos + v2(7/8, 1) * sz + v2(0, bH+0.1),      PositionAtTop
      drawLabel variables[3],       pos + v2(0.5, 1) * sz + v2(0, bH+0.1),      PositionAtTop
      drawLabel "!" & variables[0], pos + v2(0, 1/4) * sz + v2(-(bW+0.2), 0),   PositionAtRight
      drawLabel variables[0],       pos + v2(0, 3/4) * sz + v2(-(bW+0.2), 0),   PositionAtRight
      drawLabel "!" & variables[1], pos + v2(1, 1/8) * sz + v2(bW+0.2, 0),      PositionAtLeft
      drawLabel "!" & variables[1], pos + v2(1, 7/8) * sz + v2(bW+0.2, 0),      PositionAtLeft
      drawLabel variables[1],       pos + v2(1, 1/2) * sz + v2(bW+0.2, 0),      PositionAtLeft
    ...
    for x in 0..<r.cols:
      for y in 0..<r.rows:
        let d = data[y][x]
        let p = pos + v2(((x*2+1) / (r.cols*2)) * r.size.x, ((y*2+1) / (r.rows*2)) * r.size.y)
        ...
        doc.add Text mintermsText:
          Position2 p + v2(cellHalfW, cellHalfH)
