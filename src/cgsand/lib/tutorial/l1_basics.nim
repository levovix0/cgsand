...
doc.add circle(p[0], 0.1), Fill(), fg_hint

doc.add circle(p[1], 0.1), Fill(), fg_hint

doc.add circle(p[2], 0.1), Fill(), fg_hint

doc.add circle(p[3], 0.1), Fill(), fg_hint
...
proc isLevelSuccess: bool =
  ...
  doc.forEach (id: EntityId, l: LineSection2):
    sections.add l
...
proc checkLevel* =
  if isLevelSuccess():
    doc.forEach (Levelgoal, fg: var Color):
      fg = fg_success
